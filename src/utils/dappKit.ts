// Taken from https://github.com/celo-org/celo-monorepo/blob/6bf908ff14488b4101bbba9b62910904305bda7c/packages/sdk/utils/src/dappkit.ts#L148
...
export const SignTxResponseSuccess = (
  rawTxs: string[]
...
export function produceResponseDeeplink(
...
function assertString(objectName: string, key: string, value: any) {
  ...
  if (typeof value !== 'string') {
    throw new Error(
      `Expected ${objectName}[${key}] to be a string, but is ${typeof value}`
    );
  }
...
export function serializeDappKitRequestDeeplink(request: DappKitRequest) {
  ...
  switch (request.type) {
    case DappKitRequestTypes.SIGN_TX:
      params = {
        ...params,
        txs: Buffer.from(JSON.stringify(request.txs), 'utf8').toString(
          'base64'
        ),
      };
      break;
    ...
    default:
      throw new Error(
        `Invalid DappKitRequest type: ${JSON.stringify(request)}`
      );
  }
...
export function parseDappkitResponseDeeplink(
  url: string
): DappKitResponse & { requestId: string } {
...
export function parseDappKitRequestDeeplink(url: string): DappKitRequest {
  ...
  if (
    !rawParams.query.dappName ||
    !rawParams.query.callback ||
    !rawParams.query.requestId
  ) {
    throw new Error(
      `Invalid Deeplink: Does not contain meta parameters: ` + url
    );
  }
  ...
  switch (rawParams.query.type) {
    ...
    case DappKitRequestTypes.SIGN_TX:
      ...
      return {
        ...
        txs: JSON.parse(
          // @ts-ignore
          Buffer.from(rawParams.query.txs, 'base64').toString('utf8')
        ),
        ...requestMeta,
      };
    default:
